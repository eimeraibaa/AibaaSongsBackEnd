...
if (!RAILWAY_DB_URL) {
  ...
  console.error(
    '   $env:RAILWAY_DATABASE_URL="postgresql://usuario:password@host:puerto/dbname"'
  );
...
console.log(
  "📍 Host:",
  RAILWAY_DB_URL.split("@")[1]?.split("/")[0] || "desconocido"
);
...
async function runMigration() {
  try {
    ...
    if (results.length === 2) {
      ...
      console.log(
        "⚠️ No se encontraron todas las columnas. Verifica manualmente."
      );
    }
  } catch (error) {
